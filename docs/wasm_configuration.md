...
* `esplora_base_urls` - list of esplora base URLs for payment checks. The first URL is used as primary, with fallback to subsequent URLs on 5xx server errors. Also accepts a single URL string for backward compatibility.
