...
impl Sum {
    pub fn new(
        ...
        if amount == 0 {
            return Err(ProtocolValidationError::InvalidSum);
        }
    ...
    pub fn new_sat(amount: u64) -> Result<Self, ProtocolValidationError> {
        Self::new(amount, Currency::sat(), ExchangeRate::sat())
    }

    pub fn new_sat_from_str(amount: &str) -> Result<Self, ProtocolValidationError> {
        match amount.parse::<u64>() {
            Ok(num) => Self::new(num, Currency::sat(), ExchangeRate::sat()),
            Err(_) => Err(ProtocolValidationError::InvalidSum),
        }
    }
...
impl Currency {
    pub fn new(code: &str, decimals: u8) -> Result<Self, ProtocolValidationError> {
        Ok(Self {
...
fn canonicalize_code(code: &str) -> Result<[u8; 3], ProtocolValidationError> {
...
fn validate_decimals(d: u8) -> Result<u8, ProtocolValidationError> {
    // we don't support other crypto currencies
    if d > 5 {
        Err(ProtocolValidationError::InvalidCurrency)
    } else {
        Ok(d)
    }
}
...
mod tests {
    ...
    fn test_sum() {
        ...
        assert!(matches!(
            Sum::new_sat(0),
            Err(ProtocolValidationError::InvalidSum)
        ));
    }
