...
impl Email {
    pub fn new(n: impl Into<String>) -> Result<Self, ProtocolValidationError> {
        ...
        if !email_address::EmailAddress::is_valid(&s) {
            return Err(ProtocolValidationError::FieldInvalid(Field::Email));
        }
...
mod tests {
    ...
    fn test_email() {
        ...
        assert!(matches!(
            ...
            Err(ProtocolValidationError::FieldInvalid(Field::Email))
        ));
        assert!(matches!(
            Email::new("12312"),
            Err(ProtocolValidationError::FieldInvalid(Field::Email))
        ...
        assert!(matches!(
            Email::new(""),
            Err(ProtocolValidationError::FieldEmpty(Field::Email))
        ));
        assert!(matches!(
            Email::new("            "),
            Err(ProtocolValidationError::FieldEmpty(Field::Email))
