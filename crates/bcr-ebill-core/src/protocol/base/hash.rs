...
impl Sha256Hash {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Self {
        ...
        Sha256Hash(base58::encode(hash.as_slice()))
    }
...
impl FromStr for Sha256Hash {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let decoded = base58::decode(s).map_err(|_| ProtocolValidationError::InvalidHash)?;
        sha256::Hash::from_slice(&decoded).map_err(|_| ProtocolValidationError::InvalidHash)?;
...
mod tests {
    ...
    fn test_hash() {
        ...
        assert!(matches!(
            ...
            Err(ProtocolValidationError::InvalidHash)
        ));
        assert!(matches!(
