...
document.getElementById("manually_mint_bill").addEventListener("click", manuallyMintBill);
...
async function manuallyMintBill() {
    let bill_id = document.getElementById("endorse_bill_id").value;
    let endorsee = document.getElementById("endorsee_id").value;
    let measured = measure(async () => {
        return success_or_fail(await window.billApi.mint_bill({ bill_id, endorsee }));
    });
    await measured();
}
